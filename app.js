...
const pelicula3 = new peliculas({categoria: "PERROS", titulo: "Perro Feo", precio: 5, foto: `./assets/images/perroFeo.jpeg`});
const pelicula4 = new peliculas({categoria: "GATOS", titulo: "Gato Gordo", precio: 20, foto: `./assets/images/gatoGordo.jpg`});

categoria = ["PERROS", "GATOS"];
peliculasOrdenadas = [pelicula1, pelicula2, pelicula3, pelicula4];

for(let i = 0; i < categoria.length; i++){
    categorias.innerHTML += `<h2 class="main__section__title">${categoria[i]}</h2><div id="cat_peliculas" class="main__section__div"></div>`
    ...
    for(let j = 0; j < peliculasOrdenadas.length; j++){
        if(peliculasOrdenadas[j].categoria.categoria == categoria[i]){
